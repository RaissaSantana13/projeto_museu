...
import { DefaultEndHour, DefaultStartHour, EndHour, StartHour } from '../../../service/constants/constants';
import { useDictionary } from '../../../service/providers/i18n-providers';
import { CalendarEvent, EventColor } from '../../../type/type';
...
export function EventDialog({ event, isOpen, onClose, onSave, onDelete }: EventDialogProps) {
  ...
  const handleSave = () => {
    ...
    if (!allDay) {
      ...
      start_date.setHours(startHours, startMinutes, 0);
      end_date.setHours(endHours, endMinutes, 0);
    } else {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>{event?.idEvent ? dict.event.dialog.edit_event : dict.event.dialog.create_event}</DialogTitle>
          <DialogDescription className="sr-only">
            {event?.idEvent ? 'Edit the details of this event' : 'Add a new event to your calendar'}
          </DialogDescription>
        </DialogHeader>
        {error && <div className="bg-destructive/15 text-destructive rounded-md px-3 py-2 text-sm">{error}</div>}
        <div className="grid gap-4 py-4">
          <div className="*:not-first:mt-1.5">
            <Label htmlFor="title">{dict.event.dialog.title}</Label>
            <Input id="title" value={title} onChange={(e) => setTitle(e.target.value)} />
          </div>

          <div className="*:not-first:mt-1.5">
            <Label htmlFor="description">{dict.event.dialog.description}</Label>
            <Textarea id="description" value={description} onChange={(e) => setDescription(e.target.value)} rows={3} />
          </div>

          <div className="flex gap-4">
            <div className="flex-1 *:not-first:mt-1.5">
              <Label htmlFor="start-date">{dict.event.dialog.start_date}</Label>
            ...
            {!allDay && (
              <div className="min-w-28 *:not-first:mt-1.5">
                <Label htmlFor="start-time">{dict.event.dialog.start_time}</Label>
          ...
          <div className="flex gap-4">
            <div className="flex-1 *:not-first:mt-1.5">
              <Label htmlFor="end-date">{dict.event.dialog.end_date}</Label>
            ...
            {!allDay && (
              <div className="min-w-28 *:not-first:mt-1.5">
                <Label htmlFor="end-time">{dict.event.dialog.end_time}</Label>
          ...
          <div className="flex items-center gap-2">
            ...
            <Label htmlFor="all-day">{dict.event.dialog.end_time}</Label>
          </div>

          <div className="*:not-first:mt-1.5">
            <Label htmlFor="location">{dict.event.dialog.location}</Label>
